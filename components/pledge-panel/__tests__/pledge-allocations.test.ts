...
function makeItem(id: string): TopicItem {
  return {
    id,
    topic_id: "t1",
    label: id,
...
describe("computePledgeAllocations", () => {
  ...
  it("splits £10 three ways with remainder on first item", () => {
    ...
    expect(result[0]).toEqual({ topicItemId: "a", amount: 3.4 })
    expect(result[1]).toEqual({ topicItemId: "b", amount: 3.3 })
    expect(result[2]).toEqual({ topicItemId: "c", amount: 3.3 })
  })
  ...
  it("handles £1 split three ways (penny-level rounding)", () => {
    ...
    expect(result[0]).toEqual({ topicItemId: "a", amount: 0.34 })
    expect(result[1]).toEqual({ topicItemId: "b", amount: 0.33 })
    expect(result[2]).toEqual({ topicItemId: "c", amount: 0.33 })
  })

  it("excludes unselected items from the output", () => {
    const result = computePledgeAllocations(["b"], items, 20)
    const ids = result.map((r) => r.topicItemId)
  ...
  it("output order follows allItems order, not selectedIds order", () => {
    ...
    expect(result[0].topicItemId).toBe("a")
    expect(result[1].topicItemId).toBe("c")
  })
  ...
  it("works correctly when allItems contains only the selected item", () => {
    ...
    expect(result).toEqual([{ topicItemId: "x", amount: 25 }])
  })
})
