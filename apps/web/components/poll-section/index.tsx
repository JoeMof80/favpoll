...
export function PollSection({
  ...
  // Rendered twice pre-pledge: an invisible in-flow copy that keeps the
  // page's natural scroll length (and so provides the sticky runway on
  // EVERY screen, mobile included), and a visible viewport-capped copy
  // that pins beneath the stuck ribbon (founder, 2026-08-02).
  const decoyBody = (
  ...
  return (
    ...
    >
      ...
      {entitled ? (
        ...
        /* Pre-pledge: the locked block (decoy + card) PINS at a constant
           distance beneath the stuck ribbon on every screen (founder,
           2026-08-02). The invisible spacer copy keeps the page's real
        ...
        <div className="grid">
          ...
          <div className="[grid-area:1/1]">
            <div className="sticky top-[calc(var(--hero-stuck-bottom,10rem)+3rem)] md:top-[calc(var(--hero-stuck-bottom,13.75rem)+3rem)]">
              ...
              <div
                className="pointer-events-none max-h-[calc(100dvh-var(--hero-stuck-bottom,10rem)-8rem)] overflow-hidden [mask-image:linear-gradient(to_bottom,black_72%,transparent)]"
                aria-hidden="true"
              >
                <div className="opacity-60 blur-xs select-none">
                  {decoyBody}
                </div>
              </div>

              {onOpenPledgeDialog && (
                /* Only the CARD is clickable (founder, 2026-08-02) — the
                   wrapper passes events through; the card hovers with the
                   list cards' lift idiom. No inner sticky needed: the
                   whole block pins. */
                <div className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center pt-4">
                  ...
                  >
                    <span className="flex items-center justify-center gap-2 bg-primary px-4 py-2.5 text-base font-medium text-primary-foreground">
                      ...
                      {/* One universal CTA: the action is pledging YOUR
                          favourite; step 3 presents the reveal as the
                          gift. */}
