...
// ONE options factory for the rendered QR and the downloaded one (2026-08-09).
// The export exists so people can put the code on stationery favpoll will
// never template, which only works if the file they get is the code they saw.
// Two call sites building their own options is how those drift.
export function buildQrOptions({
  ...
  return {
    ...
    dotsOptions: { type: "rounded" as const, color: foreground },
    cornersSquareOptions: {
      type: "extra-rounded" as const,
      color: foreground,
    },
    cornersDotOptions: { type: "dot" as const, color: foreground },
    ...(logo
      ? {
          image: `data:image/svg+xml,${encodeURIComponent(logoSvg(foreground))}`,
          imageOptions: {
            imageSize: 0.4,
            margin: 2,
  ...
  useEffect(() => {
    ...
    import("qr-code-styling").then(({ default: QRCodeStyling }) => {
      ...
      const qr = new QRCodeStyling(
        buildQrOptions({ value, size, foreground: colors.foreground, logo })
      )
