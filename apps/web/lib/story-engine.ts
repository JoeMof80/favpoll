...
- "reveal" (guests see it only AFTER pledging): start with exactly "Our pick to start:" then ${pick ? `exactly this option, verbatim: "${pick}"` : "a real option from the list"}, then " — " (this separator is the one place an em dash is allowed) and one short, warm clause, plain and unforced, like "They watched it every Christmas Eve without fail". It need not justify the pick; the about carries the reason. No statistics, numbers, percentages, or invented quotes.`
    ...
    const truthRule = ` What you say about the person must be ordinary and believable, the kind of thing a relative would say: never a talisman, a lucky object, a superstition, a quirk invented for effect, or a place or object given agency ("walked every mile with her", "steadies him"). Every detail passes the HONOUR test: it is something the person would be glad to hear said about them by someone who loves them, at their own party or memorial. Affectionate, never mocking; nothing that makes them look odd, small or sad ("reads every beer mat on the table", "eats it straight from the box", "fumbles for the light switches"). Never announce that a favourite exists ("she had a favourite cat breed"). Never say the favourite is being kept back or withheld: simply do not name or hint at it; the closing sentence is the one promise of a reveal. Never comment on the favpoll itself ("it felt right for today", "this seemed fitting"): write about the person, not about the page.`
...
Then read it once more as someone who LOVES the person. Would they be glad to hear this said about them at their own party or memorial? Fail anything affectionate people would not say: a habit that makes them look odd, small or sad ("reads every beer mat on the table before anyone else finishes their pint", "eats it straight from the box").
