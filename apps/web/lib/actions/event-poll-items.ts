...
async function verifyOrganiser(eventPollItemId: string, userId: string): Promise<string> {
  const supabase = createAdminClient()

  const { data, error } = await supabase
    .from('event_poll_items')
    .select('event_poll_id, event_polls!inner(event_id, events!inner(id, created_by))')
