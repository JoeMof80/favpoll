...
// TWIN of apps/admin/lib/charity-commission.ts (copied 2026-09-07 for
// the wizard's any-charity register picker) — keep changes in step.
...
export async function verifyCharityNumber(
  ...
  try {
    ...
    if (res.status === 404) return { status: "not_found", registeredName: null }
    if (!res.ok) {
