...
describe('getTopics', () => {
  it('returns topics ordered by title', async () => {
    ...
    expect(topicsCalls.some((c) => c.method === 'order' && c.args[0] === 'title')).toBe(true)
  })
...
describe('updatePlaceholder', () => {
  it('fetches current placeholders, merges occasion, and updates without overwriting others', async () => {
    ...
    const { error } = await updatePlaceholder('topic-1', 'memorial', 'New about', 'New reveal')
    expect(error).toBeNull()

    const updateCall = mock.callsFor('topics').find((c) => c.method === 'update')!
...
describe('addOccasion', () => {
  it('rejects invalid occasion names without hitting the DB', async () => {
    const { error } = await addOccasion('topic-1', 'INVALID_OCCASION', 'A', 'B')
  ...
  it('accepts all 17 valid occasions', async () => {
    const validOccasions = [
      'memorial', 'tribute', 'birthday', 'retirement', 'wedding',
      'engagement', 'anniversary', 'leaving', 'graduation', 'christening',
      'achievement', 'recovery', 'award', 'promotion', 'celebration',
      'other', 'default',
    ]
  ...
  it('merges the new occasion without overwriting existing ones', async () => {
    ...
    mock.queue(null)
    const { error } = await addOccasion('topic-1', 'memorial', 'M about', 'M reveal')
    expect(error).toBeNull()
    const updateCall = mock.callsFor('topics').find((c) => c.method === 'update')!
