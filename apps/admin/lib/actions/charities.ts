...
export async function getCharities(
  ...
  let query = supabase
    .from('charities')
    .select('id, name, description, logo_url, registered_number, is_active, market, created_at')
...
export async function createCharity(input: {
  ...
  if (!VALID_MARKETS.includes(input.market)) {
    return { error: `Invalid market. Must be one of: ${VALID_MARKETS.join(', ')}.` }
  }
...
export async function updateCharity(
  ...
  if (data.logo_url !== undefined) updates.logo_url = data.logo_url.trim() || null
...
export async function reactivateCharity(id: string): Promise<{ error: string | null }> {
  const supabase = createAdminClient()
