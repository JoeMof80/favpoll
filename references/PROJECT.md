...
│   ├── editable-poll-area.tsx    -- Poll area editing: PollHeading rendered as non-interactive SectionLabel (no `onPledge` — clicking does nothing in edit mode), reveal edit Button (left-bordered italic when set, muted placeholder when empty), Tabs (amount/count), PollResults — all always visible, no blur/toggle. Uses `useFormContext<FavpollFormValues>()` internally. No showReveal/onToggleReveal props.
