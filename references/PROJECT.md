...
- **`FormInner` is the direct form host.** `favpoll-form/form-inner.tsx` owns `isGenerating` state, `handleRegenerate`, and composes `EditableHero`, `EditablePollArea`, `EditableCountdown`, and `CharityBanner` directly. A floating "Generate a suggestion" `Button` (Sparkles icon) lives in the left column after `EditablePollArea`, gated on `showSparkles = !!selectedTopics[0]?.topicId && !selectedTopics[0]?.isCustom`. Sub-components read form state via `useFormContext<FavpollFormValues>()`. `preview-panel.tsx` was deleted in `fix/sparkles-orphaned-host` — `FormInner` is the only rendering path.
