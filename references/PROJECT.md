...
│   ├── hero.tsx                  -- LandingHero: purple band, HeroTexture, fixed headline (landing.headline) + register-cycling eyebrow, demo card in traffic-light frame (scale-[0.62], sm:scale-80), count-up stats, Choose/Pledge/Reveal beat indicator
...
- **Hero demo card mechanics (implemented, PR feat/hero-demo-typewriter-disclosure; loop now in `landing/use-demo-loop.ts`).** The demo runs a 15-phase loop (`Phase` union in `scenes.ts`). Key mechanics:
  ...
  - **Scene portraits:** `protagonists.photo_url`-shaped `photo_url` fields point at `public/demo/*.jpg` (founder-generated, 800px JPEG). All six wired (james.jpg landed 2026-07-04).
