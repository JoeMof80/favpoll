...
├── header.tsx                   -- "use client"; hamburger menu on mobile (md:hidden); click-outside closes. The "New favpoll" button was removed from both desktop header and mobile menu (founder, 2026-07-15) — creation entry points are the landing CTAs and /favpolls
...
- **Hero copy simplification (2026-07-11).** Three linked decisions from one session:
  - **Headline centres the favourite: "Pick your favourite. Pledge its worth. Reveal its standing."** (`landing.headline`, 2026-07-17; rendered one sentence per line so the triad never wraps mid-beat). Supersedes the bare "Pick, pledge, reveal." (2026-07-11 → 2026-07-17), retired as semantically hollow — the guest is now the subject, the amount is framed as what the favourite is worth to them (Love pillar as pledge-value anchor), and "standing" is deliberately the record's vocabulary for what every favpoll reveals post-pledge. Grammar guard: "pledge **its** worth", never "it's". Parked follow-up: carry the worth framing into the pledge step's amount copy — that's where it changes behaviour.
...
- **Presence dial (founder concept, 2026-08-01)** — favpoll's intensity at an event is a dial: **ambient** (memorial wake — QR table cards, screen in a corner at most), **moment** (celebration — a beat where the reveal is performed, then it recedes), **rally** (fundraiser — telethon-style, the room watches the total edge toward the goal). Implications when picked up: live-display variants (quiet: no goal bar, name-forward / rally: goal thermometer as the display's protagonist), and possibly organiser-triggered reveal choreography. Full notes in `references/outreach-notes.md`.
- **Register-specific landing pages** — `/memorials`, `/celebrations`, `/fundraisers`: each shows the presence dial at the right setting and serves as the forwarding artefact for its gatekeeper (celebrants, venue coordinators, community fundraising managers). Sequencing decision (2026-08-01): build the memorial page FIRST (warm channel — Joy, funeral directors, St Luke's), learn, then template the others.
