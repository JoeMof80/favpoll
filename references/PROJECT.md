...
│   ├── command-panel.tsx         -- Floating command panel: fixed bottom-4 right-4 w-72 on desktop, full-width bottom bar on mobile. Contains: three-pick summary chips (Occasion/Charity/Topic) + 3 ResponsiveOverlay sheets, Listed/Unlisted Switch, missing-field checklist (shows "Name" for person events, "Cause" for cause events), Publish/Cancel/Settings buttons. Auto-sets isListed=false when register="remembering".
