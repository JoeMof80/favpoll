...
│   ├── poll-results.tsx
├── poll-section/
│   ├── index.tsx             -- permanent blur/reveal layout: pre-pledge shows pixel-perfect blurred decoy (blur-xs, aria-hidden, not interactive) + absolute unlock overlay button (Gift icon + "Pledge to see what {Name} loved — and how the pledges are landing." copy, cause-fallback no-name form); clicking decoy region or "Favourite {topic}" header opens PledgeDialog. Post-pledge shows real PollReveal + Tabs + RankingList. RankingList NOT mounted pre-pledge (Realtime leak prevention). Props: entitled, personalReveal, initialItems, isCause, onOpenPledgeDialog.
