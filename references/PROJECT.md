...
1. **Who fork** (one ToggleGroup, visually forked): five person options — **He / She / They** (`Mars`/`Venus`/`NonBinary` icons) / **A couple** (`Users`) / **A group** (`UsersRound`) — in a `grid-cols-2 sm:grid-cols-5` grid, then an **"OR" divider**, then **A cause** (`HeartHandshake` icon) alone below. The divider makes the subject fork visible: person options set `subject='someone'` + grouping/pronoun (He/She/They → `grouping='individual'` + pronoun; couple/group → grouping + `pronoun=undefined`); "A cause" flips `subject='cause'`, `grouping='individual'`, `pronoun=undefined` **and auto-sets `category='fundraiser'`** — the only category that derives the `cause` register (memorial/celebration would derive protagonist-presuming registers for a faceless favpoll, previously an allowed-but-incoherent state). Switching from cause back to a person **resets `category` to `null`** so the auto-set value is never silently inherited. **No default selection** — Next is gated until a who option is chosen (`whoSelected = grouping !== "individual" || subject === "cause" || pronoun !== undefined`; for cause, the auto-set category satisfies the category gate). The cause-label input has been **removed** from the wizard — organisers enter the label on the details form instead. `pronoun` rides the redirect as a `pronoun` query param (only present for `subject='someone'`) and hydrates into `defaultValues.pronoun` on the details page.
2. **Category row** (Celebration `Balloon` / Memorial `Flower2` / Fundraiser `Flag`): shown only for `subject='someone'`. For a cause it is replaced by the line "A favpoll for a cause is a fundraiser." Active ToggleGroupItem style: solid fill (`bg-primary text-primary-foreground`).
