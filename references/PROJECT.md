...
```
/                              -- Home: seven-section landing page. Section 1 = HeroDemoPanel (animated reveal sequence). Section 2 = HowItWorksThreeBeat (withhold→pledge→disclose). Section 3 = live favpolls carousel (bg-primary/5). Section 4 = the record (gated: hidden until RECORD_THRESHOLD_GBP = £500 total pledged across top favourites). Section 5 = fee model / where money goes. Section 6 = written-in-advance (will/letter of wishes). Section 7 = Honour·Charity·Love Venn + final CTA. Supabase query must NOT select `register` (dropped column — causes Supabase to return `{ data: null }` silently, showing "No live favpolls yet").
/landing-v2                    -- RETIRED (PR feat/landing-rebuild-reveal-first). Route deleted; reusable components (honour-charity-love-venn.tsx, occasion-eyebrow.tsx, how-it-works.tsx) kept in components/landing-v2/ for reuse.
...
│   ├── __tests__/hero-demo-panel.test.tsx
│   NOTE: `tagline` field renamed → `about` (PR feat/landing-rebuild-reveal-first). The stale `personal_framing` / "Is it the same as [Name]'s?" hint mechanic was removed. About text now renders in the protagonist intro zone; chips have no framing line above them — matching current product behaviour. Demo scene data aligned to EXAMPLES.md quality bar.
│   LOAD STATE: initial phase is "reveal" with bars fully populated — the reveal is visible on first paint, no timer required. After a 2.5s hold the loop begins: withhold → pick → pledge → results beat → reveal. prefers-reduced-motion: static reveal, no loop. Composed entirely from real components (ProtagonistAvatar, Chip, SectionLabel, PollReveal); no Supabase fetch.
│   BEAT SEQUENCE: Phase type includes "results" (new). Rankings (RankingBar) appear in the "results" phase as bars climb, then the PollReveal slides up in "reveal". When reveal is active, the rankings dim to opacity-0.4 to give the reveal visual focus. revealVariant slides up from y:18 (not down) with DELIBERATE timing (0.65s, easeOutExpo).
│   DEFERRED: All-time record beat (pledge feeds the all-time ranking) — not built yet. Coupled to the data-threshold logic gating the record section (RECORD_THRESHOLD_GBP = £500). Will slot in between the results beat and the reveal once the threshold is met and real numbers are available. Add as a phase after "results" in Phase type and sequences in index.tsx when the record section unlocks.
...
- **Hero demo: reveal-first load state + results beat (2026-06-24).** `HeroDemoPanel` now renders in phase `"reveal"` with bars fully populated on first paint — visitors who glance for 3 seconds see the reveal immediately. After a 2.5s hold the loop replays the withhold → pick → pledge → **results** → reveal arc. A new `"results"` phase was added to the `Phase` union; rankings (via `RankingBar`) appear and climb before `PollReveal` slides up. When the reveal is active, the rankings dim to `opacity: 0.4` to cede visual focus. `revealVariant` now slides up from below (`y: 18 → 0`) with `DELIBERATE` timing (0.65s easeOutExpo). `showReveal` in `DemoCard` was split into `showResults` (ranking bars) + `showReveal` (personal reveal) to support the two-beat sequence. `prefers-reduced-motion`: static reveal, no loop. No narration text added — the ambient micro-caption (`GUESTS' PICKS` / `[Name]'s reveal`) is retained as a quiet state indicator. All-time record beat deferred (see below).

- **Hero demo: all-time record beat (DEFERRED).** A beat showing the pledge feeding the all-time ranking ("Purple moved up") is wanted after the results beat. Blocked by two constraints: (1) the record section lower on the page is gated until RECORD_THRESHOLD_GBP = £500 — showing a fabricated record in the hero while the section says "being built" would contradict the page; (2) the beat is better with real numbers. Implementation point: add a phase after `"results"` in `Phase` type (`scenes.ts`) and sequence it in `index.tsx`; add the record-beat render in `demo-card.tsx`. Build this together with the record section unlock.
