...
**The one allowlisted exception is `apps/web/lib/og/palette.ts`** — hex mirrors of the tokens for the Open Graph cards, because Satori (next/og) draws from inline styles with no stylesheet, no CSS variables and no oklch. Change a token → change its mirror.
