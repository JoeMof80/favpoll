...
├── new-event-wizard.tsx          -- 3-step page component (Honour → Charity → Love); **full-page two-column layout** — no contained card, no `max-w-5xl` centring. `md:grid md:grid-cols-[280px_1fr] md:items-start`: left column = persistent triad rail (tense-aware prompt + step labels + subtext + opacity tiers: active 100% / past 50% / future 30%), separated by layout/space (no `border-r`); right column = step content + navigation. Mobile: left column hidden, existing step-dot `<ol>` reused as compact progress strip above step content. Uses `getWizardCopy(subject)` from `lib/wizard-copy.ts`. On completion redirects to /events/new/details?category=...&grouping=...&subject=...&charityIds=...&causeLabel=...&topicId=... (causeLabel included when subject='cause' and non-empty)
