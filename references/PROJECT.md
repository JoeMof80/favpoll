...
├── new-event-wizard.tsx          -- 3-step page component (Honour → Charity → Love); **full-page two-column layout**. `md:grid md:grid-cols-[320px_1fr] md:items-stretch`: left column = `bg-primary/10` tinted rail (`h-full`, `justify-around`), Award/Gift/Heart icons at `h-6 w-6`, `text-lg` labels, `opacity-60` for past/future; right column = `max-w-2xl` centred content. Mobile: left column hidden, labelled segmented progress strip (`<ol>` items with coloured fill + HONOUR/CHARITY/LOVE text, aria-label `"Step N of 3: <Label>"`). Navigation: `border-t`, `justify-end`, `variant="secondary"`. Uses `getWizardCopy(subject)`. On completion redirects to /events/new/details?...&causeLabel=... (causeLabel when subject='cause' and non-empty)
