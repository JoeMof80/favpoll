...
- **Printable event pack** — organiser clicks "Print pack" on OrganizerCard and gets a PDF: table/tent cards, an order-of-service insert, and an A4 poster, each carrying the favpoll's QR (already generated by OrganizerCard via qrcode.react) and the monogram pattern (`FavpollMarkGlyph`/`MonogramTiles` in `components/landing/hero-texture.tsx` are self-contained/print-ready). Zero-inventory bridge to future physical stationery; registers need distinct treatments (memorial: order of service, "in lieu of flowers" cards; celebration: invitations, table talkers, post-close thank-you cards with final rankings + total raised).
...
- **Mobile app** — future.
- **Embla carousel hydration mismatch** — `FavpollListCardCharityCarousel` (embla-carousel-react + Autoplay) restructures the DOM on mount, so `FavpollSummaryCard` throws a recoverable hydration warning wherever it renders (landing, `/charities/[id]`). Pre-existing and app-wide; React regenerates the subtree client-side so it's cosmetic, but worth a proper SSR-safe fix (mount-gate the carousel, or render a static first slide until mounted).
