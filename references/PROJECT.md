...
│   ├── fund-plan-card.tsx        -- Create-mode right-column shared-fund card: stages a seed amount (presets £10/£25/£50 + number input in a ResponsiveOverlay, "you'll be asked to pay when you publish"); staged amount lives in FavpollForm `plannedSeed` state and pre-fills SeedFundModal via its `initialAmount` prop. Payment still happens post-create (Stripe metadata needs the favpoll id) — the mandatory-pot decision is unchanged (`potAmount` still always null). Edit mode keeps the dimmed static placeholder.
...
│   ├── editable-poll-area.tsx    -- Poll area editing: PollHeading rendered with `previewPill` (2026-07-03) — the same primary pill guests see on the live page, but inert (cursor-default, no onClick; clicking still does nothing per PR #137) with a tooltip "Guests tap this to pledge once your favpoll is live"; reveal edit Button (left-bordered italic when set, muted placeholder when empty), Tabs (amount/count), PollResults — all always visible, no blur/toggle. Uses `useFormContext<FavpollFormValues>()` internally. No showReveal/onToggleReveal props.
...
- **Landing follow-ups** — james.jpg demo portrait (quota-limited; same style block, wire `photo_url` in scenes.ts ~L401).
- **Printable event pack** — organiser clicks "Print pack" on OrganizerCard and gets a PDF: table/tent cards, an order-of-service insert, and an A4 poster, each carrying the favpoll's QR (already generated by OrganizerCard via qrcode.react) and the monogram (`FavpollMonogramGlyph` in `components/landing/hero-texture.tsx` is self-contained/print-ready). Zero-inventory bridge to future physical stationery; registers need distinct treatments (memorial: order of service, "in lieu of flowers" cards; celebration: invitations, table talkers, post-close thank-you cards with final rankings + total raised).
