...
│   ├── preview-panel.tsx         -- Authoring artefact: hero fields inlined with ghost Button + Pencil edit affordances → ResponsiveOverlay draft pattern. Pre/post-reveal toggle unconditional (both person and cause). Local state: previewSuffix, previewPhoto. Always visible on mobile (stacks below; pb-52 clears command bar). Shows OnboardingPanel when no occasion selected. Accepts isGenerating, personRevealExample, onRegenerate props. **Cause events** (subject='cause'): shows causeLabel h1 (editable, 60 chars) instead of name; hides context/photo fields; About and reveal toggle shown (same as person). PollReveal uses causeLabel as protagonistFirstName. PollResults gated by revealValue only (not subject). **Person events**: full hero (name, context, photo, about, reveal toggle). subject is read from form values (not derived from register). Countdown is always `CountdownPlaceholder` (non-interactive, shows `--` with `text-muted-foreground`); closesAt is not editable here — set at publish time via CommandPanel overlay.
