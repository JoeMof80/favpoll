...
**2026-09-01 revision — the shop-window day (#607–#639,
references/session-handoff-2026-09-01.md).** /favpolls is a SHOP WINDOW
(founder call): the list card became a flip card — story front (large
avatar right, the about wrapping and scrolling), mechanic/standings back,
one fixed h-52 body with absolute faces so content can never size the
card, links-only step row ("Pledge ›" / "‹ Story"), entitled cards open
on the standings with the pledge-again gift on that step. The hidden
face is inert + pointer-events-none + opacity-faded at the flip's
midpoint (WebKit ignores backface-visibility — verify flips in
Playwright webkit). PollHeading grew its settled "FAVOURITE" eyebrow
(same size, /55 opacity — size-only tried and reverted) on every
surface, deleting the shrink-and-truncate table. Wizard: Info→Header,
Details→Settings (labels only), three-notch Visibility SegmentedControl
(first UI to set is_private), Cause fundraiser-only in the who dropdown,
the photo button IS the avatar and the crop overlay's picker is the
image itself, clickable steps everywhere (canJumpTo: passed-steps-only
in create). Cards on /favpolls, the shelves and charity pages wear
data-register (#607); card eyebrows wear register ink (#615). Keepsake:
Export image (html-to-image, stationery roadmap step 1), reached-goal
line, pinwheel corner marks in square 20×20 boxes with re-derived frame
rules (#617). Colour rollout completed (#616); register labels singular
on the last plural surfaces (#618); hero router cards quietened to
title-line + bars with neutral body, topic headers broken after
"Favourite", charity avatar in the footer (#611–#614).
...
**Phase 2 (same day) — the wizard edits.** `/favpolls/[id]/edit` renders
the SAME wizard prefilled (`WizardEditConfig`): rail entries become
buttons that jump to any step, the final button is Save
(`updateFavpoll`), no SeedFundModal. Locking: once money has moved — any
pledge, or `favpoll_pots.total_deposited > 0` — Event, Charity and Topic
render read-only summaries ("Locked — guests have already pledged.") and
`updateFavpoll` refuses category/charity/topic changes server-side. A
cause's label prefills into the wizard's Name field; who derives from
subject/grouping/pronoun. FavpollForm's edit mode is no longer routed —
it retires in Phase 3.

**Phase 3 (same day) — the in-place edit system retired.** Deleted:
`/favpolls/new/details`, FavpollForm (index/form-inner), CommandPanel,
the field overlays (name/opening-line/context/about/cause-label/goal/
close-date), EditableHero, EditablePollArea, EditableCountdown,
EditableField, GenerateExampleDialog, edit-mode-context (and the layout
provider). Survivors in components/favpoll-form/: seed-fund-modal,
hero-photo-overlay, edit-helpers (CharCounter et al.), date-helpers,
date-time-picker, schema, constants, item-add-field. The who helpers
(WhoValue/groupingForWho/subjectForWho) moved to `lib/who.ts`. The
favpoll page was already pure guest view — nothing there changed.
...
/favpolls/new                  -- The six-step publishing wizard (Event · Charity · Topic · Header · Story · Settings — Info/Details renamed 2026-09-01, keys unchanged); creates the favpoll itself (createFavpoll) and offers SeedFundModal before navigating (2026-08-31). Settings holds goal, close date and the three-notch Visibility SegmentedControl (Listed · Link only · Private — the wizard is the first UI that can set is_private, 2026-09-01)

/favpolls/[id]                 -- Favpoll page — guest pledge view + edit mode toggle
/favpolls/[id]/edit            -- The wizard, prefilled (extended-wizard Phase 2, 2026-08-31): clickable rail, Save via updateFavpoll; Event/Charity/Topic render read-only once any pledge or shared-fund deposit exists (updateFavpoll enforces the same lock server-side)
...
├── favpoll-list-card.tsx               -- The FLIP CARD on /favpolls (2026-09-01, the shop-window day, #622–#639 — see that revision block). Static frame (header without thumb, topic row with ClosingLabel level with the eyebrow, charity footer) around a FIXED h-52 body whose two faces are ABSOLUTE: story front (about wraps a float-right avatar and SCROLLS, never truncates; "Pledge ›"/"Standings ›" bottom-right), mechanic/standings back (compact LockCardContent → PledgeDialog while locked; fill-mode FavpollListCardResults once entitled, pledge-again gift beside "‹ Story"). Entitled cards open on the back. The hidden face is inert + pointer-events-none + opacity-faded at the flip's midpoint (WebKit ignores backface-visibility). Constructs FavpollPollWithItems inline; steps via buildMechanicSteps from server-derived content-free reveal flags. (Homepage carousel uses FavpollSummaryCard instead.)
├── favpoll-list-card/
...
├── favpoll-summary-card.tsx        -- Compact read-only card (no pledge UI): FavpollHeader + PollHeading (default mode — the Favourite eyebrow, 2026-09-01) + ClosingLabel (top-aligned to the eyebrow line) + FavpollListCardCharityCarousel. Wears data-register via paletteForFavpoll (#607). Used on the landing live grid (FavpollSummaryCard). Cause-aware: `protagonist` is `{ name } | null`; when `subject === 'cause'` the `cause_label` is shown as the name (a live cause favpoll used to crash the landing here). + .stories.tsx
├── organizer-card/
...
├── poll-heading.tsx             -- topicTitle + optional onPledge + inert. TWO LINES everywhere since 2026-09-01 (#626–#629): a quiet same-size "FAVOURITE" eyebrow at reduced opacity (settled after size-only was tried and reverted — the hero cards' grammar) over the topic word at full size; the old three-step shrink-and-truncate table is GONE — long topics keep their size and wrap. onPledge renders the primary Button (pledge dialog trigger); `inert` renders the text header at min-h-9 (not h-9) so the guest page's sticky offsets hold for single-line topics (2026-08-02: the lock card is the single pre-pledge CTA). Default mode: the same two lines in SectionLabel's muted tone (summary cards).
├── stripe-checkout.tsx
...
**How a page takes a palette.** `lib/register-palette.ts` → `paletteForRegister` / `paletteForFavpoll` / `paletteForSceneKind`. `components/register-scope.tsx` renders `<div data-register-page="…">` (a plain block — `display:contents` broke the router's scroll-to-top, 2026-08-31); `:root:has([data-register-page])` in `register-tokens.css` lifts it to the whole document — header, footer, logo — server-rendered, no flash, no script. Every favpoll-bound page wraps itself (favpoll page, keepsake, pack, `/live/[slug]`), each register landing page wears its own, and **`FavpollForm` wears its register live from its own values** (`useWatch` of category / grouping / subject → `deriveRegister`, 2026-08-31) so the create details page carries the wizard's colour through and the edit page follows the saved favpoll — neither page wraps the form. A single element on a mixed surface takes `data-register="…"` directly and recolours only its subtree: the home router cards, the header's register links, the wizard's three kind chips (`EventStep`), `DemoCard` (per scene kind), the reveal vignette's artefact box. **The wizard wears the register as it is chosen** (2026-08-31): `NewFavpollWizard` wraps itself in `RegisterScope` with `paletteForRegister(deriveRegister(category, grouping, subject))`, so the page is blue until a kind is picked and that kind's colour from then on — the colour arriving is the confirmation.

**The register accent system is retired** (was 2026-08-03 → 08-30: forget-me-not `--memorial`, the `*-on-band` gold/green/blue variants, `accentVar` on DemoCard/LandingHero, `accentBarClassName`, `accentClassName` on IdeasSection). The register IS the palette; there is no gold anywhere. The home router cards are **white with the register as ink** (2026-08-31, #588 — after solid bands and tinted glass were rendered and rejected): a `register-ink` utility in `globals.css` remaps `--primary-foreground` inside a register-scoped element to its own `--primary` (light) / dark page colour (dark), written at the scope rules' specificity, so every inner class that took band ink takes the register colour; hover is the register's `--accent` tint — the header links' grammar. **That grammar — white surface, register as ink, pale register tint on hover — is the house idiom for a register-coloured element on a neutral surface.**

**Paper is generated per palette too (2026-08-31):** `.paper` / `.paper-screen` — the light values pinned for print and on-screen sheets, print-dark border — come from the generator for the default and each register (`[data-register] .paper`, `:root:has([data-register-page]) .paper`), so a memorial's keepsake prints purple and a fundraiser's green; `globals.css` used to pin the original purple by hand. The pack, keepsake and live pages are page-scoped, so stationery, display and keepsake carry the register with no further work; the home journey scopes its paper, display and keepsake media to the scene's register.

**Status colours are not brand colours:** `--success` (shared fund healthy, pledge confirmations), `--warning` (+ `-muted`, `-strong`: closing ≤ 7 days, fund near limit), `--destructive` (+ `-muted`, `-strong`). A fundraiser page's brand green is one hue-step from `--success`, so "goal reached" is a state said in words there, not a green fill — resolved 2026-09-01 (#616): the charity banner's caption and the mobile footer's amount caption say "£X goal reached" once the total passes the goal.
...
- **Hero name size steps down for names that would wrap on a phone (2026-08-31).** The 2026-07-29 "one standard size" call stands for every name that fits; `heroNameSizeClass(name)` in `lib/display.ts` now returns `text-2xl sm:text-4xl` from 16 characters (`HERO_NAME_STEP_DOWN_AT`) — measured on the real cause page at 390px: the column beside the avatar is 222px and 30px medium Plus Jakarta runs ~13.5px a character. "St Mark's Hospice" (17) broke onto two lines beside Biscuit; at 24px it holds one. All three heroes and the demo card share the function; `heroNameMobileSizeClass(name)` is the phone half for the demo's phone frame.

## Outstanding TODO

- **Colour system rollout: COMPLETE (2026-09-01, #616).** OG share cards wear their register's palette (`lib/og/palette.ts` is now `OG_PALETTES` — default blue + the three registers, hexes computed from the generator's own recipe; `favpollOgCard` derives the palette from category+subject and the site card stays blue), `lib/email.ts` links moved from the retired purple to the blue default `#0058A8`, and "goal reached" is said in words on the goal surfaces. (Print pack, keepsake paper and the wizard wearing the register as it is chosen: done 2026-08-31; prototype #584 closed.)
