...
| 4b  | **Re-register Apple Pay domain**     | In the main account's LIVE mode: `POST /v1/payment_method_domains` with `favpoll.com` (the association file at `/.well-known/` ships with the app). Sandbox registration (done 2026-07-21; Apple Pay device-verified live on gamma 2026-09-07) does NOT carry over. **Raised stakes since #763:** the Express Checkout wallet row leads the payment block — if this step is missed the Apple Pay/Google Pay buttons silently vanish in production (no error, just an emptier checkout), so treat it as a launch blocker and eyeball the buttons on favpoll.com as part of step 8.                     |
...
| 7   | **Domain**                           | Point `favpoll.com` at the Vercel project; update the Stripe webhook URL (step 4) and `NEXT_PUBLIC_BASE_URL` — **temporarily set to `https://favpoll-web-gamma.vercel.app` (2026-07-28)** so QR codes/share/email links work pre-domain (it was `favpoll.com` → holding page → 404 on scan); MUST flip back to `https://favpoll.com` at this step.                                                                                                                                  |
