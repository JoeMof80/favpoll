...
| 4b  | **Re-register Apple Pay domain**     | In the main account's LIVE mode: `POST /v1/payment_method_domains` with `favpoll.com` (the association file at `/.well-known/` ships with the app). Sandbox registration (done 2026-07-21, apple_pay+google_pay active on favpoll-web-gamma.vercel.app) does NOT carry over.                     |
