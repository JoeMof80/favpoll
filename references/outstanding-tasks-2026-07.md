...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| **Stripe webhook — configure it**   | The route now EXISTS (`/api/webhooks/stripe`, 2026-07-21): create the endpoint in the Stripe dashboard (event `payment_intent.succeeded`) and set `STRIPE_WEBHOOK_SECRET` in Vercel — until then it answers 503 and reconciliation records nothing. The hourly `reconcile-payments` cron then surfaces any charged-but-unrecorded payment for manual refund/recovery. Clerk webhook secret still needs setting too. | High            |
