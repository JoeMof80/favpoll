...
| 5   | ~~**Clerk live keys + de-branding**~~    | **DONE 2026-09-07**: production instance verified on favpoll.com (5 CNAMEs at IONOS + Verify click; cert issued), custom Google OAuth credentials live (first prod sign-in via Google succeeded), live keys + fresh webhook secret deployed via CLI (dashboard blocks secret+public-prefix edits), dev-mode badge gone. DB remapped: 96 favpolls + 3 pledges → the new prod user id; 1 favpoll (another tester's) reattaches on their first prod sign-in. Residual cosmetics: "Secured by Clerk" on the sign-in card needs the paid-plan dashboard toggle; Apple OAuth deferred (needs Apple Developer membership). APPEALS_ADMIN_USER_IDS still carries the dev id until swapped. |
...
| **PF/CP legal opinion**                | qLegal DECLINED (2026-09-07): regulatory questions out of their scope; signposted CMS startups / Wiggin / Founders Law / Deborah Lawunmi (Setfords) + a pro bono sheet. Lawunmi checked: employment law, not charity — struck (the signposts were generic). Stage-1 scoping emails SENT 2026-09-07 (Bates Wells → Hood + Robbins after Carew-Jones's mat-leave OOO; Stone King via enquiry form) — Bates Wells REPLIED 2026-09-09 — Hannah Lyons (Partner, heads the Fundraising practice, works with Carew-Jones) proposes an initial call + conflict check ("what charities are involved"; clean answer: none onboarded, catalogue is consent-pending by design). Stone King still pending. the full instruction letter (`references/pfcp-instruction-letter-DRAFT-2026-09-07.md`) goes to the engaged firm only; TrustLaw possible in parallel. The consent-gate machinery is BUILT (#788–#791): `CHARITY_CONSENT_POSTURE=consent-first` flips the app to the consent-first posture the moment the opinion says so — every charity sits `pending` until admin-approved, register-added ones arrive delisted. Raised urgency now favpoll.com is live — pilot-consent path is the interim mitigation. |
