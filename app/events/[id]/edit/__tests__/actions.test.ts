...
describe("updateEvent — auth & ownership", () => {
  it("throws 'Not authenticated' when userId is null", async () => {
    ...
    await expect(updateEvent("event-1", "prot-1", makeInput()))
...
describe("updateEvent — extension validation", () => {
  it("throws when extension_count is already 2", async () => {
    ...
    mock.queue(makeEventRow({ extension_count: 2, closes_at: "2025-01-01T00:00:00Z" }))

    await expect(
      updateEvent("event-1", "prot-1", makeInput({ closesAt: "2028-01-01T00:00" }))
  ...
  it("throws when new closing date is in the past", async () => {
    ...
    await expect(
      updateEvent("event-1", "prot-1", makeInput({ closesAt: "2021-01-01T00:00" }))
  ...
  it("throws when new closing date exceeds hard_close_at", async () => {
    ...
    await expect(
      updateEvent("event-1", "prot-1", makeInput({ closesAt: "2027-06-01T00:00" }))
  ...
  it("does NOT throw when extension_count is 1 (one extension still allowed)", async () => {
    queueHappyPath(makeEventRow({ extension_count: 1, closes_at: "2025-01-01T00:00:00Z" }))
  ...
  it("increments extension_count in the event update when extending", async () => {
    ...
    await updateEvent("event-1", "prot-1", makeInput({ closesAt: "2028-01-01T00:00" }))
...
describe("updateEvent — DB writes", () => {
  ...
  it("replaces charities: deletes all then inserts new ones", async () => {
    ...
    await updateEvent("event-1", "prot-1", makeInput({ charityIds: ["c-1", "c-2"] }))

    const charityDelete = mock.callsFor("event_charities").find((c) => c.method === "delete")
    expect(charityDelete).toBeDefined()

    const charityInsert = mock.callsFor("event_charities").find((c) => c.method === "insert")!
    expect(charityInsert.args[0]).toHaveLength(2)
    expect(charityInsert.args[0][0]).toMatchObject({ charity_id: "c-1", display_order: 0 })
    expect(charityInsert.args[0][1]).toMatchObject({ charity_id: "c-2", display_order: 1 })
  })
  ...
  it("skips charity insert when charityIds is empty", async () => {
    ...
    const charityInserts = mock.callsFor("event_charities").filter((c) => c.method === "insert")
