...
if (!dbUrl) {
  console.log(
    "No DATABASE_URL found. Running migrations via Supabase REST workaround.",
  );
...
async function bootstrap() {
  ...
  if (error) {
    console.log(
      "Cannot query information_schema via PostgREST (expected):",
      error.message,
    );
  } else {
  ...
  if (pollError) {
    console.log("favpoll_polls column check:", pollError.message);
  } else {
    console.log("favpoll_polls: personal_quote/personal_reveal check passed");
  }
}
