/**
 * Migration script using pg package over Supabase pooler
 * Run with: node scripts/run-sql-migrations.mjs
 */
import pg from 'pg'
const { Client } = pg

// Supabase connection via transaction pooler (port 6543) or session pooler (port 5432)
// For DDL (ALTER TABLE), we need session mode (port 5432) on the pooler
// Password for Supabase is the database password, NOT the service role JWT
// However, Supabase also supports connecting via the service role key as password
// for the pooler in some configurations.

// The service role key payload encodes the project ref, so let's try it as password
...
// The region needs to be discovered. Let's try common ones.

const REGIONS = ['eu-west-2', 'us-east-1', 'us-west-1', 'ap-southeast-1']
...
async function main() {
  ...
  // Try session pooler (port 5432) for DDL support - needed for ALTER TABLE
  for (const region of REGIONS) {
    const host = `aws-0-${region}.pooler.supabase.com`
    client = await tryConnect(host, 5432)
    if (client) break

    // Also try transaction pooler port 6543
  ...
  if (!client) {
    ...
    client = await tryConnect(`db.${PROJECT_REF}.supabase.co`, 5432)
  }
